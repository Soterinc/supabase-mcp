...
app.post('/mcp', async (req, res) => {
  ...
  const request = JSON.stringify({ jsonrpc: '2.0', id: id || '1', method, params }) + '\n';
  ...
  try {
    ...
    const onData = (data) => {
      ...
      try {
        ...
        mcpProcess.stdout.off('data', onData); // Stop listening after first valid JSON
    ...
    setTimeout(() => {
      ...
      if (!res.headersSent) {
        res.status(500).json({ error: 'MCP server response timeout' });
      }
    }, 120000); // 120 seconds timeout
    
  } catch (error) {
    console.error('Error forwarding to MCP server:', error);
    res.status(500).json({ error: 'Failed to communicate with MCP server' });
  }
});
